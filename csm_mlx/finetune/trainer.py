...
from csm_mlx.finetune.lora import apply_lora_to_model, trainable_params
...
class BaseTrainer:
    ...
    def compute_loss(self, tokens: mx.array, masks: mx.array) -> mx.array:
        ...
        # For audio, we predict all codebooks simultaneously from text
        audio_targets = audio_tokens
        audio_masks = masks[:, :, :-1]
        ...
        c0_loss = nn.losses.cross_entropy(
            c0_logits.reshape(-1, c0_logits.shape[-1]),
            audio_targets[:, :, 0].reshape(-1),
            reduction="none"
        )
        c0_loss = (c0_loss * audio_masks[:, :, 0].reshape(-1)).sum() / audio_masks[:, :, 0].sum()
        
        # Other codebooks (through decoder)
        decoder_inputs = self.model.projection(backbone_hidden)
        decoder_hidden = self.model.decoder(decoder_inputs)
        
        other_codebooks_loss = 0.0
        for i in range(1, self.model.n_audio_codebooks):
            ci_logits = mx.matmul(decoder_hidden, self.model.audio_head[i-1])
            ci_loss = nn.losses.cross_entropy(
                ci_logits.reshape(-1, ci_logits.shape[-1]),
                audio_targets[:, :, i].reshape(-1),
                reduction="none"
            )
            ci_loss = (ci_loss * audio_masks[:, :, i].reshape(-1)).sum() / audio_masks[:, :, i].sum()
            other_codebooks_loss += ci_loss
        
        other_codebooks_loss /= (self.model.n_audio_codebooks - 1)
        
        # Combine losses
        total_loss = c0_loss + other_codebooks_loss
    ...
    def train(self, dataset: CSMDataset, batch_size: int, epochs: int, shuffle: bool = True) -> Dict:
    ...
    def save_checkpoint(self, suffix: Optional[str] = None) -> None:
        ...
        if suffix:
            checkpoint_path = os.path.join(self.checkpoint_dir, f"{prefix}_{suffix}.safetensors")
        else:
            checkpoint_path = os.path.join(self.checkpoint_dir, f"{prefix}_step_{self.step}.safetensors")
    ...
    def load_checkpoint(self, checkpoint_path: str) -> None:
        ...
        optimizer_state_path = os.path.join(os.path.dirname(checkpoint_path), "optimizer_state.json")
...
class LoRATrainer(BaseTrainer):
    ...
    def __init__(
        ...
        if self.train_embeddings:
            print("Embedding layers included in training: text_embeddings, audio_embeddings")
        else:
